...
Environment:
    ...
    TASKNOTES_API_HOST - API host (default: localhost)
"""
...
API_HOST = os.getenv("TASKNOTES_API_HOST", "localhost")
...
def list_tasks(args):
    ...
    if args.overdue:
        params["overdue"] = "true"
    # 2) Handle --scheduled for the API
    if args.scheduled:
    ...
    if args.table:
        # 3) Optionally add "Scheduled" column
        if not tasks:
            print("No tasks found.")
            return
        headers = ["Status", "Priority", "Title", "Project", "Scheduled"]
        print(f"{headers[0]:<15} {headers[1]:<10} {headers[2]:<50} {headers[3]:<30} {headers[4]:<25}")
        print("-" * 130)
        for t in tasks:
            ...
            scheduled = t.get("scheduled", "") or ""
            print(f"{status:<15} {priority:<10} {title:<50} {projects:<30} {scheduled:<25}")
...
def main():
    ...
    # 1) Add --scheduled to parser
    list_parser.add_argument("--scheduled", help="Filter by scheduled date (YYYY-MM-DD)")
